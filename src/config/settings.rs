...
pub struct AppSettings {
    ...
    pub mtls_verified_value: String,
    ...
    pub mtls_role: String,
    pub mtls_scopes: Vec<String>,
    pub tenant_isolation_enabled: bool,
    pub tenant_id_header: String,
    pub workspace_id_header: String,
    ...
    pub audit_storage_policy_path: String,
    pub audit_storage_policy_strict: bool,
    pub residency_enforcement_enabled: bool,
...
impl AppSettings {
    pub fn from_env() -> Result<Self, SettingsError> {
        ...
        Ok(Self {
            ...
            mtls_enabled: parse_env_bool("MTLS_ENABLED", false),
            mtls_verified_header: env::var("MTLS_VERIFIED_HEADER")
                .unwrap_or_else(|_| "x-client-cert-verified".to_string()),
            mtls_verified_value: env::var("MTLS_VERIFIED_VALUE")
                .unwrap_or_else(|_| "SUCCESS".to_string()),
            mtls_subject_header: env::var("MTLS_SUBJECT_HEADER")
                .unwrap_or_else(|_| "x-client-cert-subject".to_string()),
            mtls_fingerprint_header: env::var("MTLS_FINGERPRINT_HEADER")
                .unwrap_or_else(|_| "x-client-cert-fingerprint".to_string()),
            mtls_allowed_subjects: parse_csv_env("MTLS_ALLOWED_SUBJECTS", &[]),
            mtls_allowed_fingerprints: parse_csv_env("MTLS_ALLOWED_FINGERPRINTS", &[]),
            mtls_role: env::var("MTLS_ROLE").unwrap_or_else(|_| "service_account".to_string()),
            mtls_scopes: parse_csv_env("MTLS_SCOPES", &[]),
            tenant_isolation_enabled: parse_env_bool("TENANT_ISOLATION_ENABLED", false),
            tenant_id_header: env::var("TENANT_ID_HEADER")
                .unwrap_or_else(|_| "x-tenant-id".to_string()),
            workspace_id_header: env::var("WORKSPACE_ID_HEADER")
                .unwrap_or_else(|_| "x-workspace-id".to_string()),
            tenant_require_workspace: parse_env_bool("TENANT_REQUIRE_WORKSPACE", false),
            tenant_quota_requests_per_minute: parse_env_u32("TENANT_QUOTA_REQUESTS_PER_MINUTE", 0)?,
            ...
            audit_storage_policy_path: env::var("AUDIT_STORAGE_POLICY_PATH")
                .unwrap_or_else(|_| "config/audit_storage_policies.json".to_string()),
            audit_storage_policy_strict: parse_env_bool("AUDIT_STORAGE_POLICY_STRICT", false),
            residency_enforcement_enabled: parse_env_bool("RESIDENCY_ENFORCEMENT_ENABLED", false),
            deployment_region: env::var("DEPLOYMENT_REGION")
